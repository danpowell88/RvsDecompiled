...
IMPL_INFERRED("Determines which side of the door a point lies on using dot product against the door's normal vector")
...
IMPL_INFERRED("Returns true if the door has a StaticMesh, indicating it acts as a moving brush")
INT AR6IORotatingDoor::IsMovingBrush() const
{
	return StaticMesh != NULL;
}

IMPL_INFERRED("Restores cached door location after net receive; doors do not replicate position")
...
IMPL_INFERRED("Safely destroys both door actor halves then delegates to AR6InteractiveObject::PostScriptDestroyed")
...
IMPL_INFERRED("Caches door location before net receive for restoration in PostNetReceive")
...
IMPL_GHIDRA("R6Engine.dll", 0x1d550)
...
IMPL_INFERRED("R6-specific trace filter for rotating doors; respects see-through, bullet-goes-through, and mover flags")
...
IMPL_INFERRED("Returns true if an NPC approaching a closed door should cause it to swing open on touch")
...
IMPL_INFERRED("Script-callable: adds a breach actor to the door's breach list")
...
IMPL_INFERRED("Script-callable: removes a breach actor from the door's breach list")
...
IMPL_INFERRED("Script-callable: returns whether this door will open on touch by the given pawn")
