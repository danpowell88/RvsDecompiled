...
public:
	...
	*/
	...
	FVector RotateAngleAxis( INT Angle, const FVector& Axis ) const;
	FVector TransformVectorByTranspose( const FCoords& Coords ) const;
	FLOAT GetAbsMax() const;
	FLOAT GetMax() const;
	INT IsUniform();
	FLOAT& operator[]( INT i );	// Ghidra 0x101033b0
...
public:
	...
	// Arithmetic operators (Ghidra-confirmed exports; not present in SDK)
...
public:
	...
	FRotator( FLOAT InVal );	// Ghidra 0x10104060
...
public:
	...
	bool Intersect( const FBox& Other ) const;	// Ghidra 0x101050B0
	FVector GetCenter() const;					// Ghidra 0x10104F60
	FVector GetExtent() const;					// Ghidra 0x10104FC0
	void GetCenterAndExtents( FVector& Center, FVector& Extents );	// Ghidra 0x10105010
	void Init();								// Ghidra 0x10104E50
	FVector& operator[]( INT i );				// Ghidra 0x10104e90
	FBox TransformBy( const FMatrix& M ) const;	// Ghidra 0x1010A440
...
public:
	...
	// Methods defined out-of-line in UnMath.cpp.
	~FMatrix();
	FMatrix     Inverse();
	FMatrix     Transpose();
	...
	INT         operator==( FMatrix& Other ) const;
	INT         operator!=( FMatrix& Other ) const;
	void        SetIdentity();
