...
public:
	...
	// Ravenshield: TArray concatenation operators.
	TArray& operator+( const TArray& Other );
	TArray& operator+=( const TArray& Other );
...
public:
	...
	explicit FString( DOUBLE Arg, INT Digits=1, INT RightDigits=0, INT LeadZero=1 );
	...
	FString Reverse();
	INT ParseIntoArray( const TCHAR* Delim, TArray<FString>* Array );
