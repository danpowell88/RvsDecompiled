...
function Paint(Canvas C, float X, float Y)
{
	...
	if((m_HBorderTexture != none))
	{
		...
		DrawStretchedTextureSegment(C, m_fHBorderPadding, (m_fHBorderHeight + float(m_TextureRegion[0].H)), WinWidth, m_fHBorderHeight, float(m_HBorderTextureRegion.X), float(m_HBorderTextureRegion.Y), float(m_HBorderTextureRegion.W), float(m_HBorderTextureRegion.H), m_HBorderTexture);
	}
	// End:0x274
