...
function InitPageOptions()
{
	...
	__NFUN_184__(fYOffset, fYStep);
	m_pOptionMouseSens = R6WindowHScrollbar(CreateControl(Class'R6Window.R6WindowHScrollbar', fXOffset, fYOffset, __NFUN_175__(__NFUN_175__(WinWidth, fXOffset), fXRightOffset), 14.0000000, self));
	...
	__NFUN_184__(fYOffset, fYStep);
	m_pAutoAim = R6WindowTextureBrowser(CreateWindow(Class'R6Window.R6WindowTextureBrowser', fXOffset, fYOffset, __NFUN_175__(WinWidth, fXOffset), __NFUN_174__(__NFUN_174__(14.0000000, float(iAutoAimBitmapHeight)), float(iAutoAimVPadding)), self));
	m_pAutoAim.CreateSB(250, int(__NFUN_175__(m_pAutoAim.WinHeight, float(14))), 140, 14);
	m_pAutoAim.CreateBitmap(__NFUN_146__(250, iSBButtonWidth), 0, __NFUN_147__(140, __NFUN_144__(2, iSBButtonWidth)), iAutoAimBitmapHeight);
	m_pAutoAim.SetBitmapProperties(false, true, 5, false);
	m_pAutoAim.SetBitmapBorder(true, Root.Colors.White);
	m_pAutoAim.CreateTextLabel(0, 0, int(__NFUN_175__(m_pAutoAim.WinWidth, m_pAutoAim.m_CurrentSelection.WinLeft)), int(m_pAutoAim.WinHeight), Localize("Options", "Opt_AutoTarget", "R6Menu"), Localize("Tip", "Opt_AutoTarget", "R6Menu"));
...
function Notify(UWindowDialogControl C, byte E)
{
	...
	else
	{
		...
		if(C.__NFUN_303__('UWindowHScrollbar'))
		{
			...
			{
				// End:0x17C
				if(__NFUN_181__(float(m_iRefMouseSens), m_pOptionMouseSens.GetScrollBarValue()))
				{
