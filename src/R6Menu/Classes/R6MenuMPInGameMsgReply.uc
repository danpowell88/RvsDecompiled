//=============================================================================
// R6MenuMPInGameMsgReply - extracted from retail RavenShield 1.60
...
function Created()
{
	...
	m_pInGameReplyPopUp = R6WindowPopUpBox(CreateWindow(Class'R6Window.R6WindowPopUpBox', 0.0000000, 0.0000000, 640.0000000, 480.0000000));
...
function BeforePaint(Canvas C, float X, float Y)
{
	...
	if((!m_bFirstTimePaint))
	{
		...
		m_pInGameReplyPopUp.ModifyPopUpFrameWindow(Localize("Reply", "ID_HEADER", "R6RecMessages"), R6MenuRSLookAndFeel(LookAndFeel).GetTextHeaderSize(), float(m_RMsgSize.X), float(m_RMsgSize.Y), float(m_RMsgSize.W), float(m_RMsgSize.H));
	}
	return;
}
