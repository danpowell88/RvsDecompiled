...
targets_controller = [
    'AdjustFromWall', 'SetAdjustLocation', 'CheckAnimFinished', 'AcceptNearbyPath',
    'CanHear', 'CheckHearSound', 'ShowSelf', 'SetPath', 'SetRouteCache',
    'LineOfSightTo', 'CanHearSound', 'CheckEnemyVisible', 'FindPath',
    'StartAnimPoll@AController',
]

targets_pawn = [
    'physFlying', 'physSpider', 'physSwimming', 'physWalking',
    'rotateToward', 'setMoveTimer', 'startSwimming',
    'pointReachable', 'swimReachable', 'walkReachable',
]

print("=== AController stubs ===")
for t in targets_controller:
    for name, (rva, off, b) in sorted(eng.items()):
        klass = 'AController' if 'AController' in name else ''
        if t in name and klass and 'exec' not in name and 'event' not in name:
            print(f"  {name}")
            print(f"    -> {classify(b)}")
            break

print()
print("=== APawn physics/movement stubs ===")
for t in targets_pawn:
    for name, (rva, off, b) in sorted(eng.items()):
        if t in name and 'APawn' in name and 'exec' not in name and 'event' not in name:
            print(f"  {name}")
            print(f"    -> {classify(b)}")
            break
