...
# Check specific remaining one-by-one
targets = [
    'PostBeginPlay', 'StartAnimPoll', 'AttachProjector', 'DetachProjector',
    'PostNetReceive', 'PreNetReceive', 'PostNetReceiveLocation',
    # APawn voids
    'Crouch', 'UnCrouch', 'SpiderstepUp', 'StartNewSerpentine', 
    'ZeroMovementAlpha', 'clearPaths',
]

print("=== Engine.dll remaining void stubs (exact) ===")
eng_exp, _ = read_pe_exports('retail/system/Engine.dll')
for t in targets:
    matches = [(n, classify(r)) for n, (_, _, r) in eng_exp.items() if t in n and 'AActor' in n or t in n and 'APawn' in n]
    for name, cls in sorted(matches):
        if 'exec' not in name and 'event' not in name:
            print(f"  {name}")
            print(f"    -> {cls}")
