...
def classify(b):
    ...
    return f"complex [{b[:6].hex()}]"

targets = [
    'BoundProjectileVelocity', 'PostEditLoad', 'PostEditMove',
    'PostPath', 'PostRaytrace', 'PostScriptDestroyed',
    'PrePath', 'PreRaytrace', 'Spawned',
]

eng_exp, _ = read_pe_exports('retail/system/Engine.dll')
print("=== Batch 94 verification ===")
for t in targets:
    for name, (rva, off, raw) in eng_exp.items():
        if t in name and 'AActor' in name and 'exec' not in name and 'event' not in name:
            print(f"  {t}: {classify(raw)}")
            break
