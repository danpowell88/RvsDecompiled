...
# Check Engine.dll for void stubs not yet verified
targets_engine = [
    'SetGameType', 'PreNetReceive', 'PostNetReceive', 'PostNetReceiveLocation',
    'TickSpecial', 'StartAnimPoll', 'AttachProjector', 'DetachProjector', 
    'RenderEditorInfo', 'CheckOwnerUpdated',
    # UnPawn void stubs
    'Crouch', 'SpiderstepUp', 'StartNewSerpentine', 'UnCrouch',
    'ZeroMovementAlpha', 'clearPaths',
    # Also check return-value stubs
    'R6LineOfSightTo', 'R6SeePawn', 'Reachable', 'CanCrouchWalk', 'CanProneWalk',
    'Pick3DWallAdjust', 'PickWallAdjust', 'ValidAnchor', 'checkFloor',
    'findNewFloor', 'flyReachable', 'jumpReachable', 'ladderReachable',
    # CheckCircularReferences
    'CheckCircularReferences',
]

print("=== Engine.dll unverified stubs ===")
eng_exp, _ = read_pe_exports('retail/system/Engine.dll')
for name, (rva, off, raw) in sorted(eng_exp.items()):
    for t in targets_engine:
        if t in name:
            print(f"  {name}")
            print(f"    -> {classify(raw)}")
            break
