...
# Each entry: (unique_search_string_to_find_in_line, macro_to_insert)
# The search string must uniquely identify the function signature line.
insertions = [
    # --- ULodMeshInstance ---
...
# Build a set of search strings for uniqueness checking
search_strings = [s for s, _ in insertions]
...
for search, count in counts.items():
    if count != 1:
        issues.append(f"  '{search[:60]}' found {count} times")
if issues:
    print("ISSUES:")
    for i in issues:
        print(i)
else:
    print("All search strings are unique (found exactly once each)")

not_found = [s for s in search_strings if s not in counts]
if not_found:
    print("\nNOT FOUND in file:")
    for s in not_found:
        print(f"  '{s}'")
